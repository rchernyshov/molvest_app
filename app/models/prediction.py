...
class PredictionModel:
    ...
    def predict(self, date_str):
        ...
        if date_str:
            ...
            date_index = (date - self.data["Date"].min()).days
        else:
            date_index = len(self.data)
        predicted_sales = self.model.predict([[date_index]])[0]
        return {"date": date_str or "next day", "predicted_sales_kg": round(predicted_sales, 2)}

    def get_stats(self):
        # Статистика по данным
        avg_sales = self.data["Sales_kg"].mean()
        max_sales = self.data["Sales_kg"].max()
        min_sales = self.data["Sales_kg"].min()
        return {
